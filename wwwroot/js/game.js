window.DungeonCrawler = {
    ...
    init: async function (canvasId, dotnetRef) {
        ...
        try {
            ...
            this.player = await this.loadVoxelModel(await res.json(), shadowGen, { right: 'data/sword.json', left: 'data/shield.json' });
    ...
    updateHUD: function () {
        ...
        if (this.player.health <= 0) {
            this.isDead = true; this.deathText.isVisible = true;
            setTimeout(() => { if (this.dotnetRef) this.dotnetRef.invokeMethodAsync("ToggleESCMenu"); }, 2000);
        }
    },
    ...
    performSwing: function (entity) {
        ...
        const arm = entity.userData.armR;
        const anim = new BABYLON.Animation("sw", "rotation.x", 60, 0, 0);
        anim.setKeys([{frame:0, value:0}, {frame:10, value:1.5}, {frame:20, value:0}]);
        arm.animations = [anim];
        this.scene.beginAnimation(arm, 0, 20, false, 1.5, () => entity.isSwinging = false);
    },
        ...
        setTimeout(() => {
            this.entities.forEach(n => {
                ...
                if (d < 3 && dot > 0.5) {
                    ...
                    this.showDamageText("-15", n.position.clone(), "white");
    ...
    handleNPCMovement: function () {
        ...
        this.entities.forEach(n => {
            ...
            const distToPlayer = BABYLON.Vector3.Distance(n.position, this.player.position);

            if (distToPlayer < 5.0) {
                // Aggro Mode
                n.isMoving = false;
                const targetRot = Math.atan2(this.player.position.x - n.position.x, this.player.position.z - n.position.z);
                n.rotation.y = BABYLON.Scalar.LerpAngle(n.rotation.y, targetRot, 0.1);

                if (distToPlayer < 2.5 && (now - (ai.lastAtk || 0) > 3000)) {
                    this.performSwing(n);
                    ai.lastAtk = now;
                    setTimeout(() => {
                        if (BABYLON.Vector3.Distance(n.position, this.player.position) < 2.5) {
                            this.player.health -= this.currentLevel;
                            this.showDamageText("-" + this.currentLevel, this.player.position.clone(), "red");
                        }
                    }, 250);
                }
            } else {
    ...
    showDescentDialog: function () {
        ...
        btnYes.onPointerUpObservable.add(() => { panel.dispose(); this.descentUI = null; this.currentLevel++; this.isTransitioning = true; this.showDamageText("DESCENDING...", this.player.position.clone(), "cyan"); setTimeout(() => this.init(this.canvas.id, this.dotnetRef), 1000); });
    ...
    // --- Core Generation & Voxel Loader (Omitted for brevity, assume unchanged logic with proper disposal) ---
